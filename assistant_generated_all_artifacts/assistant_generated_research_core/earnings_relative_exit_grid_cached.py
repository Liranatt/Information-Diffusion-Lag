...
def close_at_or_before(prices_arg,symbol,day):
    ...
    if key not in _CLOSE_CACHE:
        ...
        _CLOSE_CACHE[key]=(ds,cs)
    ds,cs=_CLOSE_CACHE[key]
    if len(ds)==0: return None
    idx=int(np.searchsorted(ds,as_utc_day(day).value,side='right'))-1
    return float(cs[idx]) if idx>=0 else None
...
def relative_exit_sim(row,prices_arg,probs_arg,policy):
    cache_key=(CURRENT_BENCH,CURRENT_RANKER,str(RULE),str(row.get('market_id')),str(row.get('symbol')))
    cached=_SIM_CACHE.get(cache_key,'__MISS__')
    if cached!='__MISS__': return None if cached is None else dict(cached)
    base=orig_sim(row,prices_arg,probs_arg,policy)
    if base is None:
        _SIM_CACHE[cache_key]=None
        return None
    conn=float(row.get('feat_connection_strength',0.0) or 0.0)
    base['rank_score']=conn if CURRENT_RANKER=='connection' else 0.0
    if RULE is None or not is_earnings(row):
        _SIM_CACHE[cache_key]=dict(base)
        return base
    ref,days,threshold=RULE
    rs=ref_symbol(row,ref,CURRENT_BENCH)
    if not rs or rs not in prices_arg:
        _SIM_CACHE[cache_key]=dict(base)
        return base
...
rows=[]; best={}
refs=['sector','benchmark']; day_grid=[2,3,5]; thresholds=[0.0,-0.01,-0.02]
for benchmark in ['SPY','QQQ']:
    CURRENT_BENCH=benchmark; policy=policies[f'{benchmark}_hard_cap']
    for ranker in ['connection']:
