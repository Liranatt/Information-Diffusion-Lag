...
OUT=ROOT/f'local_focused_quant_seed{RUN_INDEX}'
...
src=src.split('# ---------------------------------------------------------------------------\n# Experiments.')[0]
src=src.replace("OUT = ROOT / 'benchmark_exit_search'", "OUT = ROOT / f'local_focused_quant_seed{RUN_INDEX}'")
...
def local_sobol(anchor,benchmark,n=24,seed=1):
    ...
    local=np.clip(ua + (raw[broad_n:n]-0.5)*0.20,0,1)
...
def matched_cem(anchor,benchmark,pop=8,iters=3,seed=1):
    dim=len(specs_for('baseline')); rng=np.random.default_rng(seed)
    mean=policy_to_u(anchor); std=np.full(dim,0.08)
    lower=np.maximum(0.0, mean-0.25); upper=np.minimum(1.0, mean+0.25)
    rows=[]; best=(-1e99,None,None,None); eid=0
    for it in range(iters):
        pts=np.clip(rng.normal(mean,std,size=(pop,dim)),lower,upper)
        evaluated=[]
        for u in pts:
            score,p,metrics=evaluate_u(u,'baseline',benchmark)
...
for benchmark in ['SPY','QQQ']:
    ...
    for seed_idx in range(1):
        sob_seed=7001+(0 if benchmark=='SPY' else 100)+(RUN_INDEX-1)*1000
        cem_seed=9001+(0 if benchmark=='SPY' else 100)+(RUN_INDEX-1)*1000
        sob_best,sob_trace=local_sobol(anchor,benchmark,n=24,seed=sob_seed)
        cem_best,cem_trace=matched_cem(anchor,benchmark,pop=8,iters=3,seed=cem_seed)
...
manifest={
 ...
 'optimizer_test':'24-evaluation local/global Sobol versus 24-evaluation CEM per independent seed; training-only selection',
