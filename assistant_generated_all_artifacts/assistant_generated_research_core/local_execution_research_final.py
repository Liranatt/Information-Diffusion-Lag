...
ROOT = Path('/mnt/data')
OUT = ROOT / 'local_execution_research_final'
...
for benchmark in ('SPY', 'QQQ'):
    for variant in ('baseline', 'hard_cap', 'no_follow', 'combined'):
        t0 = time.time()
        best, trace = sobol_search(variant, benchmark, n=128, seed=20260716 + (0 if benchmark == 'SPY' else 1000) + list(EXIT_SPECS).index(variant)*100)
        ...
        search_summaries.append({'benchmark': benchmark, 'variant': variant, 'algorithm': 'sobol_screen_128', 'train_score': score, 'seconds': time.time()-t0, **policy, **metrics})
    ...
    for variant in ('baseline', 'combined'):
        for seed_index, seed in enumerate((31001, 31002)):
            ...
            t0 = time.time()
            best, trace = hybrid_search(variant, benchmark, actual_seed, sobol_n=256, cem_pop=64, cem_iters=4)
    ...
    t0 = time.time()
    best_cem, trace_cem = pure_cem_search('combined', benchmark, 41001 + (0 if benchmark == 'SPY' else 10000), pop=64, iters=8)
...
for benchmark in ('SPY', 'QQQ'):
    for variant in ('hard_cap', 'no_follow'):
        chosen = search_df[(search_df.benchmark == benchmark) & (search_df.variant == variant) & (search_df.algorithm == 'sobol_screen_128')].iloc[0]
        policy_keys = set(decode(np.full(len(specs_for(variant)), 0.5), variant).keys())
        policy = {k: chosen[k] for k in policy_keys if k in chosen.index}
        policy['hold_days'] = int(policy['hold_days']); policy['max_concurrent'] = int(policy['max_concurrent'])
        policy['no_follow_days'] = int(policy['no_follow_days'])
...
manifest = {
    ...
    'search': {
        'screen': '128 scrambled Sobol points per benchmark/variant',
        'hybrid': '256 Sobol + 4x64 CEM refinement, two seeds for baseline and combined',
        'pure_cem_comparison': '8x64 pure CEM for combined',
    },
